...
for epoch in range(args.num_epochs):
    ...
    if (epoch + 1) % 5 == 0 and (epoch + 1) >= 10 and (epoch + 1) <= 60:
        if results['accuracy_0_0'] + results['accuracy_0_1'] > 1.8 and results['accuracy_1_0'] + results['accuracy_1_1'] > 1.80:
            curr_wd *= factor
        elif results['accuracy_0_0'] + results['accuracy_0_1'] < 1.80 and results['accuracy_1_0'] + results['accuracy_1_1'] < 1.80:
            curr_wd /= factor
        factor -= 1/20
        for param_group in optimizer.param_groups:
            param_group["weight_decay"] = curr_wd
        print(curr_wd)
    ...
    if epoch % args.eval_freq == 0:
        ...
        for test_name, test_loader in test_loader_dict.items():
            ...
            if test_name =='wb_val' and results['worst_accuracy'] >= best_val:
                best_val = results['worst_accuracy']
                torch.save(model, 'best_model_original.pt')
                req_epoch = epoch
